...
namespace Components.DragDrop
{
    ...
    /// of a drag. It is used by <see cref="Full.IDragSource.StartDrag"/> to determine whether a drag 
    ...
    public class DragInfo : IDragInfo
    {
        ...
        public DragInfo(object sender, MouseButtonEventArgs e)
        {
            ...
            this.DragDropCopyKeyState = Full.DragDrop.GetDragDropCopyKeyState(this.VisualSource);

            var dataFormat = Full.DragDrop.GetDataFormat(this.VisualSource);
            if (dataFormat != null)
            {
            ...
            if (sender is ItemsControl)
            {
                ...
                if (item == null)
                {
                    if (Full.DragDrop.GetDragDirectlySelectedOnly(this.VisualSource))
                    {
        ...
        public DataFormat DataFormat { get; set; } = Full.DragDrop.DataFormat;
