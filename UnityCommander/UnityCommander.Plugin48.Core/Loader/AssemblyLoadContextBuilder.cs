...
namespace UnityCommander.Plugin48.Core.Loader
{
    ...
    using UnityCommander.Plugin48.Core.LibraryModel;
    ...
    public class AssemblyLoadContextBuilder
    {
        ...
        internal void SetDefaultContext(object defaultContext)
        {
        ...
        {
            var resourceProbingPaths = new List<string>(this._resourceProbingPaths);
            foreach (var additionalPath in this._additionalProbingPaths)
            {
                foreach (var subPath in this._resourceProbingSubpaths)
                {
                    resourceProbingPaths.Add(Path.Combine(additionalPath, subPath));
                }
            }

            if (this._mainAssemblyPath == null)
            {
                throw new InvalidOperationException(
                    $"Missing required property. You must call '{nameof(this.SetMainAssemblyPath)}' to configure the default assembly.");
            }

            return new ManagedLoadContext(
                this._mainAssemblyPath,
                this._managedLibraries,
                this._nativeLibraries,
                this._privateAssemblies,
                this._defaultAssemblies,
                this._additionalProbingPaths,
                resourceProbingPaths,
                this._defaultLoadContext,
                this._preferDefaultLoadContext,
                this._lazyLoadReferences,
#if FEATURE_UNLOAD
                _isCollectible,
                _loadInMemory,
        ...
        {
            this._defaultLoadContext = context ?? throw new ArgumentException(
                                           $"Bad Argument: AssemblyLoadContext in {nameof(AssemblyLoadContextBuilder)}.{nameof(this.SetDefaultContext)} is null.");
            return this;
        }
        ...
        public AssemblyLoadContextBuilder PreferLoadContextAssembly(AssemblyName assemblyName)
        {
            ...
            {
                this._privateAssemblies.Add(assemblyName.Name);
            }
        ...
        public AssemblyLoadContextBuilder PreferDefaultLoadContextAssembly(AssemblyName assemblyName)
        {
            ...
            {
                if (assemblyName.Name != null && !this._defaultAssemblies.Contains(assemblyName.Name))
                {
                    this._defaultAssemblies.Add(assemblyName.Name);
                    var assembly = this._defaultLoadContext.LoadFromAssemblyName(assemblyName);
            ...
            //{
            //    if (name.Name == null || this._defaultAssemblies.Contains(name.Name))
            //    {
            //        // base cases
            //        continue;
            //    }

            //    this._defaultAssemblies.Add(name.Name);
            ...
            //    var assembly = this._defaultLoadContext.LoadFromAssemblyName(name);
        ...
        {
            this._lazyLoadReferences = isLazyLoaded;
            return this;
        }
        ...
        public AssemblyLoadContextBuilder AddManagedLibrary(ManagedLibrary library)
        {
            ...
            {
                this._managedLibraries.Add(library.Name.Name, library);
            }
        ...
        public AssemblyLoadContextBuilder AddNativeLibrary(NativeLibrary library)
        {
            ...
            this._nativeLibraries.Add(library.Name, library);
            return this;
        }
        ...
        public AssemblyLoadContextBuilder AddProbingPath(string path)
        {
            ...
            this._additionalProbingPaths.Add(path);
            return this;
        }
        ...
        public AssemblyLoadContextBuilder AddResourceProbingPath(string path)
        {
            ...
            this._resourceProbingPaths.Add(path);
            return this;
        }
        ...
        internal AssemblyLoadContextBuilder AddResourceProbingSubpath(string path)
        {
            ...
            this._resourceProbingSubpaths.Add(path);
            return this;
        }
    ...
    public class AssemblyLoadContext
    {
        ...
        public static AssemblyLoadContext Default { get; set; }
